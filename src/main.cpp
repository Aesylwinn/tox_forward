...
int main(int argc, const char* argv[])
{
    ...
    for (size_t i = 0; i < sizeof(nodes)/sizeof(DHTNode); ++i)
    {
        sodium_hex2bin(nodes[i].binKey, sizeof(nodes[i].binKey),
                       nodes[i].hexKey, sizeof(nodes[i].hexKey)-1, NULL, NULL,
                       NULL);
        tox_bootstrap(tox, nodes[i].ip.c_str(), nodes[i].port, nodes[i].binKey,
                      NULL);
    }
...
void handle_friend_request(Tox* tox, const uint8_t* publicKey,
...
void handle_friend_message(Tox* tox, uint32_t friendNum, TOX_MESSAGE_TYPE type,
                           const uint8_t* message, size_t length,
                           void* userData)
{
